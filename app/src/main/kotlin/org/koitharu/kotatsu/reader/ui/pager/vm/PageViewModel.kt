...
class PageViewModel(
	 ...
	 * AVIF/animated-WebP variant), which normally means the fallback below re-encodes it into a
	 * static PNG so *something* renders. That fallback decodes into a single [android.graphics.Bitmap],
	 * which can only ever hold one frame -- so for a genuinely multi-frame source it would silently
	 * destroy every frame but the first, overwriting the cached file in place with no way back.
	 * Guard against that: if the source is actually animated, surface the original error instead so
	 * the page can be routed to the animated-capable view, rather than "fixing" it by deleting its
	 * animation. */
