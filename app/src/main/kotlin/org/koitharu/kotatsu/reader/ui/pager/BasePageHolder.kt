...
abstract class BasePageHolder<B : ViewBinding>(
	...
	private var showAnimatedJob: Job? = null
	...
	private fun playAnimatedImage() {
		...
		showAnimatedJob?.cancel()
		showAnimatedJob = lifecycleScope.launch { showAnimatedImage(uri) }
	}
	...
	private fun showAnimatedImage(source: ImageSource, autoStart: Boolean = true) {
		...
		showAnimatedJob?.cancel()
		showAnimatedJob = lifecycleScope.launch { showAnimatedImage(uri, autoStart) }
	}

	/** Learns the source's true pixel size *before* Coil ever touches [animatedImageView], and
	 * applies it to the webtoon container right away. Without this, the container is first
	 * measured while [ssiv] still reports no size at all (its decoder rejected this source), falls
	 * back to the full RecyclerView height, and only gets corrected once the async Coil load
	 * finishes -- which briefly (and on slow connections, not-so-briefly) shows the frame stretched
	 * to fill the screen. [AnimatedImageDetector.peekSize] reads just the header, so this is cheap
	 * enough to await inline before kicking off the real load. */
	private suspend fun showAnimatedImage(uri: Uri, autoStart: Boolean = true) {
		animatedPlayButton.isVisible = false
		...
		val request = ImageRequest.Builder(context)
			...
			.listener(
				onSuccess = { _, result ->
					// Fallback in case peekSize() above failed (unusual/corrupt header) but the full
					// decode still succeeded -- keeps the container correctly sized either way.
					val image = result.image
					if (size == null && image.width > 0 && image.height > 0) {
						(itemView as? WebtoonFrameLayout)?.contentAspectRatio =
	...
	private fun resetAnimatedImage() {
		...
		(itemView as? WebtoonFrameLayout)?.contentAspectRatio = 0f
