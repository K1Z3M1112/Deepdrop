...
abstract class BasePageHolder<B : ViewBinding>(
	...
	private fun showAnimatedImage(uri: Uri, autoStart: Boolean = true) {
		...
		val request = ImageRequest.Builder(context)
			...
			.listener(
				onSuccess = { _, result ->
					// ssiv never learns this source's real size (its decoder rejected it outright),
					// so in webtoon mode the frame would otherwise fall back to the full screen
					// height instead of the animation's actual (often much shorter) aspect ratio.
