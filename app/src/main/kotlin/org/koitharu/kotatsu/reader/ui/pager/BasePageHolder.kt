...
abstract class BasePageHolder<B : ViewBinding>(
	 ...
	 * routed to [animatedImageView] instead, where Coil can drive playback. [ssiv] is still fed
	 * the same source and kept in the hierarchy (just hidden) so webtoon scroll-range math, which
	 * depends on its measured size, keeps working unchanged. */
	private fun checkAnimated(source: ImageSource) {
		...
		animatedCheckJob = lifecycleScope.launch {
			...
			if (isAnimated && settings.isPageAnimationsEnabled) showAnimatedImage(uri) else resetAnimatedImage()
		}
	}

	private fun showAnimatedImage(uri: Uri) {
